...
process_hisat2(){
        ...
        read -p "Please enter the base name for the index files: " HISAT_INDEX
        # Exit if there is no input entered
        if [[ -z $HISAT_INDEX ]] ; then
                return
        else
                TEST_FILE=($HISAT_INDEX*)
                while [ ! -f $TEST_FILE ] ; do
                        read -p "$HISAT_INDEX base files do not exist. Please try again [hit return to exit]: " HISAT_INDEX
                        if [[ -z $HISAT_INDEX ]] ; then
                                return
                        fi
                done
        ...
        do
                ...
                hisat_command="$HISAT2 -x $HISAT_INDEX -1 $firstOfPair -2 $secondOfPair -S /dev/null  2> $TEMPFILE"
                eval $hisat_command
